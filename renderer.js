...
async function loadDiscordBotControls() {
  ...
  const welcome = result.config.welcome || {};
  $('#discordBotWelcomeEnabled').checked = Boolean(welcome.enabled);
  $('#discordBotWelcomeChannelId').value = welcome.channelId || '';
...
async function saveDiscordBotSettings() {
  const result = await window.coreShiftAPI.saveDiscordBotConfig({
    ...
    welcome: { enabled: $('#discordBotWelcomeEnabled').checked, channelId: $('#discordBotWelcomeChannelId').value.trim() }
  });
  if (result.success) {
